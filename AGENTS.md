...
- `js/riscv32-unknown-none-elf-*.js` — Emscripten-generated JS glue for WASM modules (exposed as globals on `window`)
- `js/*.wasm` — WebAssembly binaries (as-new, ld-new, objcopy, objdump)
- `js/codemirror.js`, `js/gas.js` — CodeMirror editor with GAS syntax mode (UMD, falls back to globals)
...
The `.wasm` files and JS glue (`riscv32-unknown-none-elf-*.js`) are compiled from binutils 2.46.0 using Emscripten targeting `wasm32`. Requires `emsdk` in PATH.
...
- No framework, no bundler — plain JS with native ES6+ features (dynamic `import()`, `async`/`await`, etc.)
